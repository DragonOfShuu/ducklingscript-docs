...
:::danger
Not Recommended: Please use `FROM` or `IMPORT` instead.

Using `START` is almost like copying and pasting the code from one file to another. This 
can lead to code duplication and makes it harder to maintain your code. Instead, consider 
using `FROM` or `IMPORT` to reference other files in a more structured way.
:::
